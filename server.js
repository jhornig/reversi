...
/* When a webpage disconnects alert everyone in the room */
      ...
      /* payload:
        ...
        */
        ...
        /* payload:
            ...
            {
              'result': 'success',
              'socket_id': the socket id of the person who are playing with
          ...
          */
          ...
          /* payload:
              ...
              if successful a success message will be followed by a game_update messages
              play_token_response:
              {
                'result': 'success',
              }
            ...
            */
            ...
            /* Check that the player has previously registered */
              ...
              if(('undefined' === typeof row) || row <0 || row > 7){
                var error_message = 'server didn\'t specify a valid row, command aborted';
                log(error_message);
                socket.emit('play_token_response', {
                  result: 'fail',
                  message: error_message
              ...
              if(('undefined' === typeof column) || column <0 || column > 7){
                var error_message = 'server didn\'t specify a valid column, command aborted';
                log(error_message);
                socket.emit('play_token_response', {
                  result: 'fail',
                  message: error_message
              ...
              if(('undefined' === typeof color) || !color || (color != 'white' && color != 'black')){
                var error_message = 'server didn\'t specify a valid color, command aborted';
                log(error_message);
                socket.emit('play_token_response', {
                  result: 'fail',
                  message: error_message
...
function send_game_update(socket, game_id, message){
  ...
  if((games[game_id].player_white.socket != socket.id) && (games[game_id].player_black.socket != socket.id)){
    console.log('Player isn\t assigned a color: '+socket.id);
